...
export default function GenerateTestCasesPage() {
  ...
  const [files, setFiles] = useState<string[]>(["mercury-app-v2.4.apk"]);
  ...
  if (stage === "results") {
    return (
      <Box sx={{ px: { xs: 2, md: 4 }, py: 3, pr: { md: assistantOpen ? 45 : 4 } }}>
        <Stack direction={{ xs: "column", lg: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", lg: "end" }} spacing={2} sx={{ mb: 2 }}><Box><Button onClick={() => setStage("compose")} sx={{ ...buttonSx, color: "#658091", px: 0, mb: 1 }}>← Back to inputs</Button><Typography variant="h4" sx={{ fontWeight: 800 }}>Authentication test suite <Chip label="AI GENERATED" size="small" sx={{ ml: 1, bgcolor: "#e4f7f5", color: "#097d80", fontSize: 10 }} /></Typography><Typography variant="body2" color="text.secondary">Generated from <b>{files[0] ?? "your sources"}</b> and your guidance</Typography></Box><Stack direction="row" spacing={1} flexWrap="wrap"><Button onClick={() => setAssistantOpen((value) => !value)} sx={{ ...buttonSx, border: "1px solid #d7e1e5" }}>✦ {assistantOpen ? "Hide" : "Open"} AI copilot</Button><Button onClick={() => notify("Regenerating suggestions…")} sx={{ ...buttonSx, border: "1px solid #d7e1e5" }}>↻ Regenerate</Button><Button onClick={() => notify("Suite saved")} variant="contained" sx={{ ...buttonSx, bgcolor: teal }}>Save suite</Button></Stack></Stack>
