...
export default function GenerateTestCasesPage() {
  ...
  const [generationProfile, setGenerationProfile] = useState<"smoke" | "feature" | "regression" | "deep_regression">("feature");
  ...
  const generateMutation = useMutation({ mutationFn: () => testCasesApi.generate(selectedProjectId, selectedRequirementIds, undefined, generationProfile), onSuccess: (res) => setResult(res.data) });
  const { data: liveRun } = useQuery({ queryKey: ["generation-run", result?.id], queryFn: () => testCasesApi.getRun(result!.id).then((res) => res.data), enabled: Boolean(result?.id), refetchIntervalInBackground: true, refetchInterval: (query) => ACTIVE_STATUSES.includes(((query.state.data as GenerationRun | undefined)?.status ?? result?.status) ?? "") ? 2500 : false });
  ...
  return <Stack spacing={3}>
    ...
    <Card sx={{ borderRadius: 3 }}><CardContent>
      <Typography variant="h6" sx={{ mb: 1 }}>Build an AI test plan</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Choose the release depth first. QTXpert will keep the plan visible while it generates and validates coverage.</Typography><TextField select fullWidth label="Generation profile" value={generationProfile} onChange={(e) => setGenerationProfile(e.target.value as typeof generationProfile)} sx={{ mb: 2 }}><MenuItem value="smoke">Smoke · critical paths, fastest feedback</MenuItem><MenuItem value="feature">Feature · balanced coverage</MenuItem><MenuItem value="regression">Regression · broad release coverage</MenuItem><MenuItem value="deep_regression">Deep regression · maximum scenario depth</MenuItem></TextField><Typography variant="h6" sx={{ mb: 1 }}>Select requirements</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Leave all unchecked to generate from every requirement in this project.</Typography>
      ...
      {generateMutation.isError && <Alert severity="error" sx={{ mt: 2 }}>{(generateMutation.error as any)?.response?.data?.detail || (generateMutation.error as Error)?.message || "Generation failed. Check the AI provider configuration."}</Alert>} {liveRun === undefined && result && !generateMutation.isPending && <Alert severity="info" sx={{ mt: 2 }}>The run is still being synchronized. QTXpert will continue polling and recover the session automatically.</Alert>}
