    ...
    return (
      <Box sx={{ maxWidth: 980, mx: "auto", px: { xs: 2, md: 5 }, py: 6 }}>
        ...
        <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", sm: "center" }} spacing={2} sx={{ mt: 3, pt: 2, borderTop: "1px solid #e2e9eb" }}><Typography variant="caption" color="text.secondary">✦ You can edit every case after generation</Typography><Stack direction="row" spacing={1}><Button onClick={() => setStage("compose")} sx={{ ...buttonSx, border: "1px solid #d7e1e5" }}>← Edit inputs</Button><Button variant="contained" onClick={() => setStage("results")} sx={{ ...buttonSx, bgcolor: teal }}>Build my test suite →</Button></Stack></Stack>
        </Paper>
      </Box>
    );
  }
  ...
  if (stage === "results") {
    return (
      <Box sx={{ px: { xs: 2, md: 4 }, py: 3, pr: { md: assistantOpen ? 45 : 4 } }}>
        <Stack direction={{ xs: "column", lg: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", lg: "end" }} spacing={2} sx={{ mb: 2 }}><Box><Button onClick={() => setStage("compose")} sx={{ ...buttonSx, color: "#658091", px: 0, mb: 1 }}>← Back to inputs</Button><Typography variant="h4" sx={{ fontWeight: 800 }}>Authentication test suite <Chip label="AI GENERATED" size="small" sx={{ ml: 1, bgcolor: "#e4f7f5", color: "#097d80", fontSize: 10 }} /></Typography><Typography variant="body2" color="text.secondary">Generated from <b>{files[0] ?? selectedSource ?? "your guidance"}</b> and your guidance</Typography></Box><Stack direction="row" spacing={1} flexWrap="wrap"><Button onClick={() => setAssistantOpen((value) => !value)} sx={{ ...buttonSx, border: "1px solid #d7e1e5" }}>✦ {assistantOpen ? "Hide" : "Open"} AI copilot</Button><Button onClick={() => setStage("analyze")} sx={{ ...buttonSx, border: "1px solid #d7e1e5" }}>↻ Regenerate</Button><Button onClick={saveSuite} variant="contained" sx={{ ...buttonSx, bgcolor: teal }}>Save suite</Button><Button onClick={startNewChat} sx={{ ...buttonSx, border: "1px solid #d7e1e5" }}>＋ New chat</Button></Stack></Stack>
