...
export default function GenerateTestCasesPage() {
  ...
  const { data: liveRun, isError: liveRunError } = useQuery({ queryKey: ["generation-run", result?.id], queryFn: () => testCasesApi.getRun(result!.id).then((res) => res.data), enabled: Boolean(result?.id), refetchIntervalInBackground: true, refetchInterval: (query) => ACTIVE_STATUSES.includes(((query.state.data as GenerationRun | undefined)?.status ?? result?.status) ?? "") ? 2500 : false });
