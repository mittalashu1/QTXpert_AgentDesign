...
export default function GenerateTestCasesPage() {
  ...
  const { data: liveRun } = useQuery({ queryKey: ["generation-run", result?.id], queryFn: () => testCasesApi.getRun(result!.id).then((res) => res.data), enabled: Boolean(result?.id) && ACTIVE_STATUSES.includes(result?.status ?? ""), refetchInterval: 2500 });
