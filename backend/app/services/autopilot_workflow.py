...
ALLOWED_TRANSITIONS: dict[str, frozenset[str]] = {
    ...
    # An explicit source re-analysis can begin while an older job is in its
    # discovery phase, especially after a Render restart invalidated the local
    # analysis snapshot. Allow that bounded restart instead of failing before
    # regenerated cases can be handed back to Runtime Discovery.
    ...
    "partial": frozenset({"exploring", "execution_ready", "running", "plan_pending_review", "plan_approved", "cases_pending_review", "cases_approved"}),
