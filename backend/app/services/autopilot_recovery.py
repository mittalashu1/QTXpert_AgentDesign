"""Restart recovery for in-flight Autopilot analysis jobs.
...
re-materializes mobile artifacts when needed, and restarts analysis from the
beginning. Website jobs need no materialization because their URL is durable.
Re-running analysis is intentionally idempotent: generated analysis replaces the
same job's result instead of creating duplicate jobs or test portfolios.
"""
...
async def _resume_one(settings: Settings, job_id: str) -> None:
    """Restore one in-flight job and restart its analysis safely."""
    ...
    async with AsyncSessionLocal() as db:
        ...
        is_web = str(record.target_kind or "android") == "web"
        if not is_web and record.repository_asset_id is None:
            record.status = "failed"
            record.stage = "failed"
            record.progress = 100
            record.error = (
                "Analysis was interrupted by a service restart and this legacy mobile artifact is not "
                "available in the Upload Repository. Upload the APK/IPA once more."
            )
            await db.commit()
            return

        target = service.root / record.job_id / Path(record.filename or ("application.apk" if not is_web else "website")).name
        ...
        if not is_web:
            try:
                ...
                if record is not None:
                    ...
                    record.stage = "failed"
                    record.progress = 100
                    record.error = f"Could not restore mobile artifact after service restart: {type(exc).__name__}: {exc}"[:1000]
                    await db.commit()
                logger.exception("Autopilot mobile recovery failed job_id=%s", job_id)
                return
    ...
    if not is_web:
        changes["apk_path"] = str(target)
    await service.update_job(job_id, **changes)
